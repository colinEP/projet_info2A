...
# Adapted November 2018
# No restriction on usage nor dissemination
########################################
#
# Help : see readme file
# Bugs and questions : castagne@imag.fr , francois.portet@imag.fr
########################################
...
#	diff $DIFF_OPTIONS $out_file_name $res_file_name > /dev/null
	diffmipsObjdump $out_file_name $res_file_name > /dev/null
	...
	then
		...
		diffmipsObjdump $out_file_name $res_file_name | echoOutputQuiet
	fi
...
menuPostTest () 
{
	...
	do
		...
		1)
			...
			;;
		2)
			mipsObjdump "$res_file_name"
			;;
		3)
			mipsObjdump "$out_file_name"
			;;
		4)
		#	diff --context=2 $DIFF_OPTIONS "$out_file_name" "$res_file_name"
			diffmipsObjdump $out_file_name $res_file_name 
			;;
		"")
			return
		esac
